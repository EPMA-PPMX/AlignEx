...
type ModuleKey = "base" | "skills" | "benefits";
...
export default function OrganizationModulesManagement() {
    ...
    try {
      const [modsResult, orgsResult] = await Promise.all([
        supabase
          ...
          .select(
            "id, organization_id, module_key, module_name, is_active, license_key, activation_date, expiry_date, purchased_date, renewal_date",
          )
      ...
      const orgMap = new Map(
        (orgsResult.data || []).map((org: { id: any }) => [org.id, org]),
      );

      const modulesWithOrganization = (modsResult.data || []).map(
        (mod: { organization_id: unknown }) => ({
          ...mod,
          organization: orgMap.get(mod.organization_id) || null,
        }),
      );

      setModules(modulesWithOrganization);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!form.organization_id) {
      showNotification("Select an organisation", "error");
      return;
    }
  ...
  const handleToggleActive = async (mod: OrgModule) => {
    try {
      ...
      if (error) throw error;
      showNotification(
        `Module ${!mod.is_active ? "activated" : "deactivated"}`,
        "success",
      );
      loadData();
    } catch (err: any) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">
          Manage module subscriptions across all organisations.
        </p>
      </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((mod) => {
                  ...
                  const isExpired =
                    mod.expiry_date && new Date(mod.expiry_date) < new Date();
                  const expiringSoon =
                    mod.expiry_date &&
                    !isExpired &&
                    (new Date(mod.expiry_date).getTime() - Date.now()) /
                      (1000 * 60 * 60 * 24) <=
                      30;
                    ...
                    >
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`p-2 rounded-lg ${meta.bg} flex-shrink-0`}
                          >
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${mod.is_active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                        >
                      ...
                      <td className="px-5 py-4">
                        {mod.expiry_date ? (
                          <div
                            className={`flex items-center gap-1 text-xs font-medium ${isExpired ? "text-red-600" : expiringSoon ? "text-amber-600" : "text-gray-600"}`}
                          >
                            ...
                            {expiringSoon && (
                              <span className="ml-1 bg-amber-100 text-amber-600 px-1.5 py-0.5 rounded text-xs">
                                Soon
                              </span>
                            )}
                          </div>
                  ...
                  {MODULE_KEYS.map((key) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          !editingModule && handleModuleKeyChange(key)
                        }
                        disabled={!!editingModule}
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    Enable to grant organisation access to this module
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, license_key: e.target.value }))
                    }
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, purchased_date: e.target.value }))
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                  />
                </div>
