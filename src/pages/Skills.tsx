...
import { Award, TrendingUp, Star, Save, Calendar } from 'lucide-react';
...
export default function Skills() {
  ...
  const getCategoryStats = () => {
    if (!skills.length) return { total: 0, rated: 0, expert: 0, intermediate: 0 };

    const rated = skills.filter((s) => {
      const level = getUserSkillValue(s.id, 'proficiency_level', 'None');
      return level !== 'None';
    });

    const expert = skills.filter(
      (s) => getUserSkillValue(s.id, 'proficiency_level') === 'Expert'
    );

    const intermediate = skills.filter(
      (s) => getUserSkillValue(s.id, 'proficiency_level') === 'Intermediate'
    );
  ...
  return (
    <div className="p-8 space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-900">Skills Assessment</h1>
        <p className="text-slate-600 mt-2">Rate your proficiency in various skills and track your certifications</p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-slate-600">Skills Rated</span>
            <Star className="w-5 h-5 text-amber-500" />
          </div>
          <p className="text-2xl font-bold text-slate-900">{stats.rated}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-slate-600">Expert Level</span>
            <TrendingUp className="w-5 h-5 text-green-500" />
          </div>
          <p className="text-2xl font-bold text-slate-900">{stats.expert}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
          ...
          <p className="text-2xl font-bold text-slate-900">{stats.intermediate}</p>
        </div>
      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        handleSkillChange(skill.id, 'years_of_experience', parseFloat(e.target.value));
                        setEditingSkill(skill.id);
                      }}
                  ...
                  <div className={skill.is_certifiable ? 'md:col-span-2' : 'md:col-span-2'}>
