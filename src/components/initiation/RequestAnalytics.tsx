...
export default function RequestAnalytics({ requests }: RequestAnalyticsProps) {
  const statusColors: {
    ...
    "Pending Approval": {
      bg: "bg-gradient-to-br from-[#C76F21] to-[#FAAF65]",
      text: "text-white",
      icon: Clock,
    },
    Approved: {
      bg: "bg-gradient-to-br from-[#276A6C] to-[#5DB6B8]",
      text: "text-white",
      icon: CheckCircle,
    },
    Rejected: {
      bg: "bg-gradient-to-br from-[#D43E3E] to-[#FE8A8A]",
      text: "text-white",
      icon: XCircle,
    },
  ...
  const totalCost = requests.reduce(
    (sum, req) => sum + (Number(req.initial_estimated_cost) || 0),
    0,
  );

  const avgCost = requests.length ? totalCost / requests.length : 0;
  const requestsWithCost = requests.filter(
    (r) => r.initial_estimated_cost,
  ).length;

  const totalDuration = requests.reduce(
    (sum, req) => sum + (req.estimated_duration || 0),
    0,
  );
  const avgDuration = requests.length > 0 ? totalDuration / requests.length : 0;
  const requestsWithDuration = requests.filter(
    (r) => r.estimated_duration,
  ).length;
            ...
            return (
              <div key={status} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <StatusIcon
                    className={`w-4 h-4 ${iconColors[status] || "text-gray-600"}`}
                  />
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-gradient-to-br from-[#7e22ce]/10 to-[#a855f7]/10 rounded-lg p-4 border border-[#7e22ce]/20">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-[#7e22ce] font-medium">
                Total Cost
              </span>
            </div>
      ...
      <div className="bg-widget-bg rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-gradient-to-br from-[#7e22ce]/10 to-[#a855f7]/10 rounded-lg p-4 border border-[#7e22ce]/20">
            ...
            <p className="text-xl font-bold text-[#7e22ce]">
              {totalDuration.toLocaleString("en-US")}{" "}
            ...
            <div key={index}>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className="bg-gradient-to-r from-[#7e22ce] to-[#a855f7] h-2 rounded-full transition-all"
                  style={{
                    width: `${(range.count / maxDurationCount) * 100}%`,
                  }}
                />
              </div>
            </div>
        ...
        <p className="text-xs text-gray-500 mt-4">
          {requestsWithDuration} of {requests.length} requests have duration
          estimates
        </p>
      </div>
    </div>
