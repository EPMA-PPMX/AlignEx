...
export default function BenefitTracking({ projectId }: BenefitTrackingProps) {
  ...
  const loadData = async () => {
    try {
      ...
      // Load impacts and priorities separately
      const [impactsResult, prioritiesResult, benefitsResult] =
        await Promise.all([
          supabase
            .from("project_priority_impacts")
            .select("id, priority_id, planned_impact")
          ...
          supabase
            ...
            .select(
              "id, project_id, priority_id, month_year, estimated_benefit_value, actual_benefit_value, notes, created_at, updated_at",
            )
            .eq("project_id", projectId)
      ...
      const prioritiesMap = new Map(
        (prioritiesResult.data || []).map((priority: { id: any }) => [
          priority.id,
          priority,
        ]),
      );
      ...
      // Join data
      const formattedImpacts = (impactsResult.data || []).map(
        (impact: any) => ({
          id: impact.id,
          priority_id: impact.priority_id,
          planned_impact: impact.planned_impact,
          priority: prioritiesMap.get(impact.priority_id) || null,
        }),
      );

      setPriorityImpacts(formattedImpacts);

      setMonthlyBenefits(benefitsResult.data || []);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const priorityImpact = priorityImpacts.find(
        (p) => p.priority_id === selectedPriority,
      );
      const estimatedValue = priorityImpact?.planned_impact
        ? parseFloat(priorityImpact.planned_impact.replace(/[^0-9.-]+/g, ""))
        : 0;
        ...
        <div className="flex items-start">
          ...
          <div>
            <h3 className="text-lg font-semibold text-yellow-900 mb-2">
              No Priority Impacts Defined
            </h3>
            <p className="text-yellow-800">
              This project does not have any organizational priority impacts
              defined. Priority impacts are typically set during the project
              initiation phase.
            </p>
          </div>
        </div>
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Target: {impact.priority?.target_value}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-lg font-semibold text-gray-900">
                    {impact.planned_impact}
                  </p>
        ...
        <div className="bg-widget-bg rounded-lg shadow-md p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  >
                    ...
                    {priorityImpacts.map((impact) => (
                      <option
                        key={impact.priority_id}
                        value={impact.priority_id}
                      >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Click the calendar icon and use arrow buttons to change year
                  </p>
                </div>
          ...
          {monthlyBenefits.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-sm">
                Add your first entry to start tracking actual benefits.
              </p>
            </div>
          ) : (
