...
export default function StatusReport() {
  ...
  const saveStatusReport = async (status: 'draft' | 'submitted') => {
    ...
    await Promise.all([
      saveRisks(statusReportId),
      saveIssues(statusReportId),
      saveChangeRequests(statusReportId),
      saveBudget(statusReportId),
      saveTasks(statusReportId),
      saveTeam(statusReportId),
      saveBenefits(statusReportId),
    ]);
  };

  const saveRisks = async (statusReportId: string) => {
    await supabase.from('status_report_risks').delete().eq('status_report_id', statusReportId);

    if (reportData.risks.length > 0) {
      const { error } = await supabase.from('status_report_risks').insert(
  ...
  const saveIssues = async (statusReportId: string) => {
    await supabase.from('status_report_issues').delete().eq('status_report_id', statusReportId);

    if (reportData.issues.length > 0) {
      const { error } = await supabase.from('status_report_issues').insert(
  ...
  const saveChangeRequests = async (statusReportId: string) => {
    await supabase.from('status_report_change_requests').delete().eq('status_report_id', statusReportId);

    if (reportData.changeRequests.length > 0) {
      const { error } = await supabase.from('status_report_change_requests').insert(
  ...
  const saveBudget = async (statusReportId: string) => {
    await supabase.from('status_report_budget').delete().eq('status_report_id', statusReportId);

    if (reportData.budget.length > 0) {
      const { error } = await supabase.from('status_report_budget').insert(
        reportData.budget.map((item) => ({
          status_report_id: statusReportId,
          ...item,
        }))
      );
  ...
  const saveTasks = async (statusReportId: string) => {
    await supabase.from('status_report_tasks').delete().eq('status_report_id', statusReportId);

    if (reportData.tasks.length > 0) {
      const { error } = await supabase.from('status_report_tasks').insert(
  ...
  const saveTeam = async (statusReportId: string) => {
    await supabase.from('status_report_team_availability').delete().eq('status_report_id', statusReportId);

    if (reportData.team.length > 0) {
      const { error } = await supabase.from('status_report_team_availability').insert(
  ...
  const saveBenefits = async (statusReportId: string) => {
    await supabase.from('status_report_benefits').delete().eq('status_report_id', statusReportId);

    if (reportData.benefits.length > 0) {
      const { error } = await supabase.from('status_report_benefits').insert(
        reportData.benefits.map((benefit) => ({
          status_report_id: statusReportId,
          ...benefit,
        }))
      );
